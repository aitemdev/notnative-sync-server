# NotNative Electron – Keybindings / Atajos

> PC usa `Ctrl`, macOS usa `Cmd`. // PC uses `Ctrl`, macOS uses `Cmd`.

## English
- **Navigation & panels**: `Ctrl+E` cycle views (edit → split → preview); `Ctrl+T` open sidebar nav; `Esc` closes sidebar/nav/chat/backlinks; `Ctrl+Shift+C` toggle AI chat; `Ctrl+Shift+L` toggle backlinks panel; `Ctrl+,` open settings; `Ctrl+Shift+N` open Quick Note.
...
- **Editor**: `Ctrl+S` save note; `Ctrl+K` with selection opens AI bubble; `[[` trigger wikilink autocomplete (use `Tab`/`Enter` to select, `Esc` to cancel).
- **Wikilinks**: Type `[[` to autocomplete note names; `Ctrl+Click` on wikilink text in editor to open note (in insert mode); valid links shown in blue, broken in red; in preview mode, wikilinks appear as clickable buttons.
...
- **By context**: Global `Ctrl+E`, `Ctrl+T`, `Ctrl+Shift+C`, `Ctrl+Shift+L`, `Ctrl+,`, `Ctrl+Shift+N`, `Ctrl+F`, `Alt+F`, `Esc`. Editor `Ctrl+S`, `Ctrl+K`, `Ctrl+E`, `[[`. Search overlay `Enter`/`Shift+Enter`, `Esc`, `Ctrl+F`/`Alt+F`. Quick Note `Ctrl+Enter`, `Esc`. Sidebar quick search `Ctrl+P`.

## Español
- **Navegación y paneles**: `Ctrl+E` alterna vistas (edición → dividido → preview); `Ctrl+T` abre navegación del sidebar; `Esc` cierra sidebar/nav/chat/backlinks; `Ctrl+Shift+C` muestra/oculta chat AI; `Ctrl+Shift+L` muestra/oculta panel de enlaces; `Ctrl+,` abre ajustes; `Ctrl+Shift+N` abre "Nota rápida".
...
- **Editor**: `Ctrl+S` guarda la nota; `Ctrl+K` con selección abre el bubble de AI; `[[` activa autocompletado de wikilinks (usa `Tab`/`Enter` para seleccionar, `Esc` para cancelar).
- **Wikilinks**: Escribe `[[` para autocompletar nombres de notas; `Ctrl+Click` en el texto del wikilink en el editor abre la nota (en modo insert); enlaces válidos en azul, rotos en rojo; en modo preview, los wikilinks aparecen como botones clickeables.
